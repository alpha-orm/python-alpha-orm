...
class MySQLQueryBuilder(implements(QueryBuilderInterface)):
    ...
    DATA_TYPE = {'float': 'double', 'str': 'text',
                 'bool': 'smallint', 'int': 'int'}
    ...
    def updateRecord(tablename, dict_map, id):
        ...
        for column in columns:
            ...
            if isinstance(colVal, AlphaRecord):
                continue
            colVal = 1 if colVal is True else 0 if isinstance(
                colVal, bool) else colVal
    ...
    def insertRecord(tablename, dict_map):
        ...
        for column in columns:
            ...
            if column != 'id' and isinstance(getattr(dict_map, column), AlphaRecord):
                column += '_id'
            sql += f"`{column}`"
            sql += ') VALUES (' if column.replace('_id',
                                                  '') == columns[-1] else ','
        for column in columns:
            if column in ['_tablename', '_id']:
                continue
