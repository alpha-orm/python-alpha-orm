...
class MySQLGenerator (implements(GeneratorInterface)):
            ...
            if col['Field'] in columns_record:
                ...
                if col['Field'] != 'id' and ((col['Type'].startswith(MySQLQueryBuilder.DATA_TYPE[get_type(getattr(alpha_record, col['Field']))])) == False):
                    if col['Field'] in AlphaORM.DATA_TYPES:
                        ...
                        updated_columns[col['Field']] = MySQLQueryBuilder.DATA_TYPE[get_type(
                            getattr(alpha_record, col['Field']))]
                        ...
                        if get_type(getattr(alpha_record, col['Field'])) in ['int', 'float']:
                            ...
                            updated_columns[col['Field']
                                            ] = MySQLQueryBuilder.DATA_TYPE['str']
    ...
    def creatNewColumns(dict_map, alpha_record, tablename):
        ...
        for col in dict_map:
            ...
            if isinstance(col, AlphaRecord):
                ...
                raise RuntimeError(
                    'Values can only be number, string or boolean')
            else:
                if isinstance(getattr(alpha_record, col), AlphaRecord):
                    ...
                    new_columns[col] = MySQLQueryBuilder.DATA_TYPE[get_type(
    ...
    def columns(columns_db, alpha_record, base=True):
        ...
        updated_columns, existing = MySQLGenerator.checkColumnUpdates(
            columns_db, columns_record, alpha_record)
        diff_array = array_diff(columns_record, existing)
        new_columns = MySQLGenerator.creatNewColumns(
            diff_array, alpha_record, alpha_record.getTableName())
